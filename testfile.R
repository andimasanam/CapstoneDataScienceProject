...
news_text <- readLines("en_US/en_US.news.txt")
twit_text <- readLines("en_US/en_US.twitter.txt")
blog_text <- readLines("en_US/en_US.blogs.txt")
...
write.table(sampleNews, "en_samples/samplenews.txt", row.names = FALSE, col.names = FALSE)

set.seed(12345)
sampleTweet <- sample(twit_text, length(twit_text)*0.05)
write.table(sampleTweet, "en_samples/sampletweet.txt", row.names = FALSE, col.names = FALSE)

set.seed(12345)
sampleBlog <- sample(blog_text, length(blog_text)*0.05)
write.table(sampleBlog, "en_samples/sampleblog.txt", row.names = FALSE, col.names = FALSE)

## After creating the sample files, we incorporate them into a single corpus
en_corpus <- Corpus(DirSource(directory = "en_samples/"))
